...
In this section, we present an extension of \zspa, called \zspaa which lets a (trusted) third-party auditor, \aud, help identify misbehaving clients in the \zspa and generate a vector of random polynomials with a specific structure. Specifically, the functionality that \zspaa computes is defined as 
%
$f^{\st \zspaa}(\underbrace{\bot,..., \bot}_{\st m}, b, \bm\zeta)\rightarrow (\underbrace{(k, g, q),..., (k, g,q)}_{\st m},$ $\bm\mu^{\st (1)},..., \bm\mu^{\st (m)})$, where $g$ is the Markle tree's root built on the pseudorandom values $z_{\st i, j}$, $q$ is the hash value of the key used to generate the pseudorandom values, $m\geq 2$, $b$ is constant value, $\bm\zeta$ is a random polynomial of degree $1$, each $\bm\mu^{\st (j)} $ has the form $\bm\mu^{\st (j)} = \bm\zeta\cdot \bm\xi^{\st (j)}-\bm\tau^{\st (j)}$,  $\bm\xi^{\st (j)}$ is a random polynomial of degree $b-2$, and $\bm\tau^{\st (j)}=\sum\limits^{\st b-1}_{\st i=0}z_{\st i,j}\cdot x^{\st i}$. 
