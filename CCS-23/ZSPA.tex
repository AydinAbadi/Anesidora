...
\zspa will be used in \withFai to let clients $\{A_{\st 1},...,A_{\st m}\}$ provably agree on sets of pseudorandom values. Every client uses the elements of these sets (as coefficients) to generate a distinct pseudorandom polynomial such that when all clients' pseudorandom polynomials are added it would result in $0$, due to \zspa's property.  Each of these polynomials will be used by a client to blind the messages it sends to the smart contract, to protect the privacy of the plaintext message (from \aud, D, and the public). To compute the sum of the plaintext messages, one can easily sum the blinded messages, which removes the blinding polynomials. 
