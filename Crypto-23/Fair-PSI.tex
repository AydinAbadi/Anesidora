...
%\begin{remark}
One may be tempted to replace $\withFai$ with a scheme in which all clients send their encrypted sets to a server (potentially semi-honest and plays \aud's role) which computes the result in a privacy-preserving manner.  We highlight that the main difference is that in this (hypothetical) proposal the server is \emph{always involved};  whereas, in $\withFai$, \aud is offline if the clients behave honestly.% and it is invoked only when the contract detects misbehaviours.  
%\end{remark}
