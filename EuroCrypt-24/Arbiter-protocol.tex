...
Now, we introduce an extension of \zspa, called \zspaa, designed to enable a (trusted) third-party auditor, \aud, to assist in the identification of misbehaving clients within the \zspa and generate a vector of random polynomials with a specific structure. Specifically, the functionality that \zspaa computes is defined as 
%
...
If\ $\zspa$ is secure, $\mathtt{H}$ is second-preimage resistant, and the correctness of $\mathtt{PRF}$, $\mathtt{H}$, and Merkle tree hold,  then \zspaa securely computes $f^{\st \zspaa}$ in the presence of $m-1 $ malicious adversaries.% or (ii) a semi-honest auditor. 
\end{theorem}
